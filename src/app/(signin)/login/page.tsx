...
export default function Login() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if(!credentials.email || credentials.email === '' || !credentials.password || credentials.password === '') return
        ...
        <div className="grid min-h-screen place-items-center bg-linear-to-r from-[#22C1C3] to-[#FDBB2D]">
            <Card className="h-120 w-[90vw] sm:w-125">
                <CardHeader className="text-4xl mt-3 text-[#268889] mb-8 font-extrabold flex flex-col items-center justify-center">
                    Login
                    <CardDescription className="font-bold">Log in and find what u need</CardDescription>
                </CardHeader>
                <form ref={ref} onSubmit={handleSubmit}>
                  ...
                  <CardFooter className="grid gap-10 mt-4 mb-3">
                    <Link href={"/forgot-password"}>
                      <div className="text-[#268889] text-sm underline flex justify-center">Forgot ur password?</div>
                    </Link>
                    <div className="flex w-full justify-center gap-4 flex-col sm:flex-row">
                      <Button size={"lg"} type="submit" className="bg-[#3eb2b4] hover:bg-[#FDBB2D]">Log in</Button>
                        <Link href='/registry' className="mb-3 sm:mb-0">
                          <Button size={"lg"} className="bg-[#3eb2b4] hover:bg-[#FDBB2D] w-full">Sign up</Button>
                        </Link>
                    </div>
                  </CardFooter>
                </form>
