...
export default function CategoriesComponent() {
  const {categories, loading, addCategories, updateCategories, deleteCategories} = fetchCategories()
  ...
  return (
      ...
      <div className="w-full flex flex-col sm:flex-row justify-between items-center">
        <div>
          {alert && <AlertComponent loading={loading} item={editingItem} onDelete={handleDelete} onCancel={handleCancel} />}
        ...
        <div>
          <Button onClick={handleAdd} className="bg-[#3eb2b4] hover:bg-[#FDBB2D] mt-2 xl:mt-0">Add Category<Plus /></Button>
        </div>
      </div>
      {
        loading ?
        ...
        categories.map((category) => (
          <Card key={category.id} className="w-full h-full p-0 m-0 shadow-gray-400"><Link href={`/categories/${category.slug}`} key={category.id}>
            ...
            <CardFooter className="pb-4 text-sm font-medium text-gray-500">
              <span>
                ...
                <div className="flex gap-2" aria-label="actions">
                  ...
                  <Button variant={'ghost'} onClick={() => handleEdit(category)}><Edit /></Button>
                </TooltipTrigger>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                  <Button variant={'ghost'} onClick={() => handleConfirmDelete(category)}><Trash /></Button>
                </TooltipTrigger>
      ...
      <CategoriesModal key={editingItem ? editingItem.id : 'new'} data={editingItem} onSuccess={handleSuccess} onSave={handleSave}/>
    </Dialog>
  </div>
      }
      </>
